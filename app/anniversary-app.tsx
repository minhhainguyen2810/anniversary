...
import { countdown, dateDifference, formatDate, formatDateInput, isSpecialToday, localIsoDate, parseDateInput, upcomingOccurrences } from "@/lib/date-engine";
...
export default function AnniversaryApp({ mode, anniversaries: initial, household: initialHousehold, authError }: { mode: Mode; anniversaries: AppAnniversary[]; household: Household; authError?: string }) {
  ...
  return <main className="app-shell">
    ...
    <section className="content-wrap">
      ...
      </> : <DaysView anniversaries={anniversaries} onAdd={() => { setEditing(null); setModal("add"); }} onEdit={(item) => { setEditing(item); setModal("edit"); }} onDelete={removeAnniversary} household={household} onAddMember={() => setMemberModal(true)} onCopy={() => { navigator.clipboard?.writeText(household?.invite_code ?? ""); setToast("Invite code copied."); }} onSignOut={signOut} />}
    </section>
    ...
    {memberModal && household && <MemberInviteModal household={household} onClose={() => setMemberModal(false)} onCopy={() => { navigator.clipboard?.writeText(household.invite_code); setToast("Invite code copied."); }} />}
...
function AnniversaryModal({ mode, initial, onClose, onSave }: { mode: "add" | "edit"; initial: AppAnniversary | null; onClose: () => void; onSave: (name: string, date: string) => void }) {
  ...
  const [date, setDate] = useState(initial ? formatDateInput(initial.anniversary_date) : "");
  const [error, setError] = useState("");
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><form className="modal-card" onSubmit={(event) => { event.preventDefault(); const normalized = parseDateInput(date); if (!normalized || normalized > localIsoDate()) { setError("Enter a valid date as dd/mm/yyyy, not in the future."); return; } setError(""); onSave(name, normalized); }}><div className="modal-heading"><div><p className="eyebrow">{mode === "add" ? "A new memory" : "Update a memory"}</p><h2>{mode === "add" ? "Add an anniversary" : "Edit anniversary"}</h2></div><GlassButton label="Close" onClick={onClose}><X size={19} /></GlassButton></div><label>Name<input autoFocus required maxLength={80} value={name} onChange={(event) => setName(event.target.value)} placeholder="Our first date" /></label><label>Date<input required type="text" inputMode="numeric" maxLength={10} pattern="\d{2}/\d{2}/\d{4}" value={date} onChange={(event) => setDate(event.target.value.replace(/[^\d/]/g, "").slice(0, 10))} placeholder="dd/mm/yyyy" aria-describedby="date-format-help" /><span id="date-format-help" className="field-help">Use dd/mm/yyyy</span></label>{error && <p className="auth-message" role="alert">{error}</p>}<button className="primary-button wide" type="submit">{mode === "add" ? "Save anniversary" : "Save changes"}</button></form></div>;
}

function MemberInviteModal({ household, onClose, onCopy }: { household: Exclude<Household, null>; onClose: () => void; onCopy: () => void }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="modal-card member-modal"><div className="modal-heading"><div><p className="eyebrow">Share your household</p><h2>Add a member</h2></div><GlassButton label="Close" onClick={onClose}><X size={19} /></GlassButton></div><p className="muted">Share this invite code. The new member signs in, chooses “Join a household”, and enters the code.</p><div className="member-code"><strong>{household.invite_code}</strong><GlassButton label="Copy invite code" onClick={onCopy}><Copy size={17} /></GlassButton></div><button className="primary-button wide" onClick={onCopy}><Copy size={17} /> Copy invite code</button></section></div>;
}
